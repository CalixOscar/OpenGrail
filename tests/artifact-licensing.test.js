...
test("Artifact Licensing & Dimension Invariants Suite", async (t) => {
  ...
  await t.test("Exactly 1,089 artifacts in graph.json with valid metadata and Commons URLs", () => {
    assert.equal(graphArtifacts.length, 1089, "graph.json must contain exactly 1089 artifacts");
  ...
  await t.test("public/artifacts/ contains exactly one file per imageUrl with no orphans in either direction", async () => {
    ...
    assert.equal(
      diskFiles.length,
      1089,
      `public/artifacts/ must contain exactly 1089 files on disk, found ${diskFiles.length}`
    );
  ...
  await t.test("Every artifact file is .webp", async () => {
    ...
    assert.equal(webpFiles.length, 1089, "Expected exactly 1089 .webp files on disk");
  });
  ...
  await t.test("Decoded dimensions are within tier (max 1600 for detail: high, max 640 for standard)", async () => {
    ...
    assert.equal(highDetailCount, 78, "Expected exactly 78 high-detail artifacts");
    assert.equal(standardDetailCount, 1011, "Expected exactly 1011 standard-detail artifacts");
    assert.deepEqual(decodeErrors, [], "All 1,089 WebP artifacts must decode without error");
    assert.deepEqual(
      dimensionViolations,
      [],
