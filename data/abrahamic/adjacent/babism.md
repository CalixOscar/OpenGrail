---
id: babism
title: Babism
cluster: Abrahamic
color: '#4da7c8'
era_start: 1844 CE
epistemic_tier: academic_consensus
summary: >-
  A nineteenth-century Iranian religion founded by the Bab, emerging from a
  Twelver Shi'a messianic environment.
aliases:
  - Babi Faith
  - Babi Religion
canonical_texts:
  - Qayyum al-Asma
  - Persian Bayan
  - Arabic Bayan
relations:
  - target: twelver-shia
    type: branch_of
    certainty: academic_consensus
  - target: shaykhism
    type: influenced_by
    certainty: academic_consensus
origin_year: 1844
origin_geo:
  lat: 29.6103
  lng: 52.5311
  place_name: 'Shiraz, Fars, Iran'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: In 1844 Sayyid Ali Muhammad of Shiraz declared
    himself the Bab, first understood as a gate to the hidden Imam and later as
    an independent messenger inaugurating a new revelation.
  - >-
    Distinctive Practice: Babi scripture reworks Shi'a messianic expectation
    through new laws, symbolic exegesis, sacred calendars, and an emphasis on
    progressive revelation. Most surviving Babis later followed Baha'u'llah and
    became Baha'is; a smaller Azali community followed Subh-i Azal.
  - >-
    Core Orientation: A nineteenth-century Iranian religion founded by the Bab,
    emerging from a Twelver Shi'a messianic environment.
sources:
  - title: 'Encyclopaedia Britannica: "Babism"'
    url: 'https://www.britannica.com/topic/babism'
  - title: 'Oxford Reference — search results for "Babism"'
    url: 'https://www.oxfordreference.com/search?q=Babism'
  - title: 'Cambridge Core — search results for "Babism"'
    url: 'https://www.cambridge.org/core/search?q=Babism'
artifacts:
  - title: "Shrine of the Báb and terrace gardens on Mount Carmel in Haifa"
    imageUrl: "/artifacts/babism-1.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Yefe_Nof_-_Bahai_terraces,_followed_by_the_German_Colony_(Ben_Gourion_av)_and_in_the_far_end_on_the_right_the_Dagon_silo.JPG"
    provenance: "FLLL; CC BY-SA 4.0"
    period: "Image dated 2013"
  - title: "Autograph Haykal (pentagram tablet) handwritten by the Báb"
    imageUrl: "/artifacts/babism-2.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Bab-Star-Tablet-Haykal.jpg"
    provenance: "The Báb, founder of Bábism; Public domain"
    period: "Mid-19th century"
    detail: high
---

# Babism

## Historical context

In 1844 Sayyid Ali Muhammad of Shiraz declared himself the Bab, first understood as a gate to the hidden Imam and later as an independent messenger inaugurating a new revelation. His movement spread rapidly in Qajar Iran, met intense clerical and state opposition, and experienced armed confrontations and massacres. The Bab was executed in 1850, but his writings anticipated another messenger to come.

## Distinctives and comparative notes

Babi scripture reworks Shi'a messianic expectation through new laws, symbolic exegesis, sacred calendars, and an emphasis on progressive revelation. Most surviving Babis later followed Baha'u'llah and became Baha'is; a smaller Azali community followed Subh-i Azal. Babism's branch edge records emergence from a Twelver setting, not continued membership in Twelver Islam. Its own claims explicitly moved beyond Islamic legal authority.
