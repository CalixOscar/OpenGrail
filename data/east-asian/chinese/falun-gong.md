---
id: falun-gong
title: Falun Gong
cluster: East Asian
color: '#63ce9d'
era_start: 1992 CE
epistemic_tier: academic_consensus
summary: >-
  A modern Chinese cultivation movement combining meditative exercises with
  moral and cosmological teachings presented by Li Hongzhi.
canonical_texts:
  - Zhuan Falun
  - Falun Gong
  - Li Hongzhi's lectures
relations:
  - target: buddhism
    type: influenced_by
    certainty: academic_consensus
  - target: daoism
    type: influenced_by
    certainty: academic_consensus
origin_year: 1992
origin_geo:
  lat: 39.9042
  lng: 116.4074
  place_name: 'Beijing (Forbidden City), China'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: Li Hongzhi introduced Falun Gong during China's
    early-1990s qigong boom. Large-scale public practice and an increasingly
    independent organizational presence brought conflict with the Chinese state,
    which banned and violently suppressed the movement in 1999.
  - >-
    Distinctive Practice: Falun Gong joins five exercises with moral cultivation
    organized around truthfulness, compassion, and forbearance. Its scriptures
    describe karma, spiritual transformation, and a complex cosmos in language
    that draws on but substantially reworks Buddhist, Daoist, and qigong
    sources.
  - >-
    Core Orientation: A modern Chinese cultivation movement combining meditative
    exercises with moral and cosmological teachings presented by Li Hongzhi.
sources:
  - title: 'Encyclopaedia Britannica: "Falun Gong"'
    url: 'https://www.britannica.com/topic/falun-gong'
  - title: 'Oxford Reference: Overview of Falun Gong'
    url: 'https://www.oxfordreference.com/search?q=Falun%20Gong'
  - title: 'Stanford Encyclopedia of Philosophy: East Asian Traditions & Thought'
    url: 'https://plato.stanford.edu/search/searcher.py?query=Falun%20Gong'
artifacts:
  - title: "The Falun emblem of Falun Dafa"
    imageUrl: "/artifacts/falun-gong-1.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Asian_swastika_and_yin-and-yang_motif.jpg"
    provenance: "Lori Blaja; CC BY-SA 3.0"
    period: "Image dated 2009"
  - title: "Mass Falun Gong group meditation practice in Guangzhou, mid-1990s"
    imageUrl: "/artifacts/falun-gong-2.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Origins-GuangzhouPractice.jpg"
    provenance: "ClearWisdom.net; CC BY-SA 3.0"
    period: "Image dated 2012"
---

# Falun Gong

## Historical context

Li Hongzhi introduced Falun Gong during China's early-1990s qigong boom. Large-scale public practice and an increasingly independent organizational presence brought conflict with the Chinese state, which banned and violently suppressed the movement in 1999. Communities subsequently developed extensive transnational media, advocacy, and practice networks.

## Distinctives and comparative notes

Falun Gong joins five exercises with moral cultivation organized around truthfulness, compassion, and forbearance. Its scriptures describe karma, spiritual transformation, and a complex cosmos in language that draws on but substantially reworks Buddhist, Daoist, and qigong sources. Classification varies between cultivation practice, new religious movement, and religion; those analytical labels should remain distinct from state propaganda or the movement's own self-description.
