---
id: cargo-cult-movements
title: So-Called Cargo Cult Movements
cluster: Indigenous & Diasporic
color: '#698a70'
era_start: late 19th century CE
epistemic_tier: minority_scholarly
summary: >-
  A contested scholarly category for distinct Oceanic prophetic movements
  responding to colonial power and exchange.
canonical_texts:
  - Movement-specific oral traditions
  - Prophetic songs and ritual corpora
  - Colonial and community historical records
relations:
  - target: christianity
    type: syncretized_with
    certainty: academic_consensus
    citation: 'Holger Jebens, ed., Cargo, Cult, and Culture Critique (2004)'
origin_year: 1850
origin_geo:
  lat: -19.4577
  lng: 169.3057
  place_name: 'Tanna Island (Mount Yasur), Vanuatu'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: Anthropologists and colonial officials applied “cargo
    cult” to many unrelated prophetic, political, and ritual movements in
    Melanesia and nearby regions. These ...
  - >-
    Distinctive Practice: Particular movements used Christian prophecy,
    ancestors, kastom, new ritual, military imagery, or cooperative organization
    in different combinations. Some expec...
  - >-
    Core Orientation: A contested scholarly category for distinct Oceanic
    prophetic movements responding to colonial power and exchange.
sources:
  - title: 'Encyclopaedia Britannica: "So-Called Cargo Cult Movements"'
    url: 'https://www.britannica.com/topic/so-called-cargo-cult-movements'
  - title: 'Oxford Reference: Overview of So-Called Cargo Cult Movements'
    url: >-
      https://www.oxfordreference.com/search?q=So-Called%20Cargo%20Cult%20Movements
  - title: UNESCO Intangible Cultural Heritage & Indigenous Knowledge Systems
    url: 'https://ich.unesco.org/en/search?q=So-Called%20Cargo%20Cult%20Movements'
artifacts:
  - title: "John Frum ritual red cross shrine at Sulphur Bay, Tanna"
    imageUrl: "/artifacts/cargo-cult-movements-1.jpg"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:JohnFrumCrossTanna1967.jpg"
    provenance: "Tim Ross; CC BY 3.0"
    period: "Image dated 1967"
    description: "This image shows John Frum ritual red cross shrine at Sulphur Bay, Tanna, selected as a defining visual reference for So-Called Cargo Cult Movements."
  - title: "John Frum ceremonial flag-raising ritual on Tanna"
    imageUrl: "/artifacts/cargo-cult-movements-2.jpg"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:John_Frum_effigy_1960.jpg"
    provenance: "David Attenborough; Public domain"
    period: "Image dated 1960"
    description: "This image shows John Frum ceremonial flag-raising ritual on Tanna, selected as a defining visual reference for So-Called Cargo Cult Movements."
---

# So-Called Cargo Cult Movements

## Historical context

Anthropologists and colonial officials applied “cargo cult” to many unrelated prophetic, political, and ritual movements in Melanesia and nearby regions. These arose amid missionization, labor recruitment, racial hierarchy, war, and radically unequal access to wealth. Grouping them by an alleged desire for Western goods often made Indigenous theories of justice and sovereignty appear irrational while treating colonial extraction as normal.

## Distinctives and comparative notes

Particular movements used Christian prophecy, ancestors, kastom, new ritual, military imagery, or cooperative organization in different combinations. Some expected material transformation, but “cargo” was never their single concern. The node is minority-tier because scholars increasingly criticize the category itself. Users should navigate to named communities such as John Frum and interpret each movement through its own language, leaders, and political history.
