---
id: zurvanism
title: Zurvanism
cluster: Iranian
color: '#d9ad2b'
era_start: c. 4th century BCE
epistemic_tier: minority_scholarly
summary: >-
  A debated Zoroastrian current in which Zurvan, Time, framed the relation of
  Ohrmazd and Ahriman.
canonical_texts:
  - Avesta (shared Zoroastrian scripture)
  - Middle Persian Zurvanite testimonia
  - Armenian and Syriac polemical accounts
relations:
  - target: zoroastrianism
    type: branch_of
    certainty: minority_scholarly
    citation: 'R. C. Zaehner, Zurvan: A Zoroastrian Dilemma (1955)'
origin_year: -350
origin_geo:
  lat: 32.8359
  lng: 52.1292
  place_name: 'Isfahan, Iran'
extinct_year: 900
key_tenets:
  - >-
    Foundational Doctrine: Zurvanism is a scholarly name for doctrines reported
    especially by late antique Armenian, Syriac, Greek, and Middle Persian
    witnesses. These accounts make Zurva...
  - >-
    Distinctive Practice: The category may represent several speculative
    tendencies rather than one denomination, and scholars dispute its antiquity,
    coherence, and place within Zoroastr...
  - >-
    Core Orientation: A debated Zoroastrian current in which Zurvan, Time,
    framed the relation of Ohrmazd and Ahriman.
sources:
  - title: 'Encyclopaedia Britannica: "Zurvanism"'
    url: 'https://www.britannica.com/topic/zurvanism'
  - title: 'Oxford Reference: Overview of Zurvanism'
    url: 'https://www.oxfordreference.com/search?q=Zurvanism'
  - title: 'Brill Handbook of Religious Traditions: Zurvanism'
    url: 'https://brill.com/search?q=Zurvanism'
artifacts:
  - title: "Luristan bronze relief depicting Zurvan giving birth to twins"
    imageUrl: "/artifacts/zurvanism-1.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:%27Kizil_Kuh%27_Plaque_LACMA_M.76.97.174.jpg"
    provenance: "Wikimedia Commons; Public domain"
    period: "Image dated 1000"
    description: "This image shows Luristan bronze relief depicting Zurvan giving birth to twins, selected as a defining visual reference for Zurvanism."
  - title: "Sasanian rock relief at Naqsh-e Rajab"
    imageUrl: "/artifacts/zurvanism-2.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Kartir.jpg"
    provenance: "dynamosquito; CC BY-SA 2.0"
    period: "Image dated 2008"
    description: "This image shows Sasanian rock relief at Naqsh-e Rajab, selected as a defining visual reference for Zurvanism."
---

# Zurvanism

## Historical context

Zurvanism is a scholarly name for doctrines reported especially by late antique Armenian, Syriac, Greek, and Middle Persian witnesses. These accounts make Zurvan, often understood as infinite Time, the parent or ground of the opposed spirits Ohrmazd and Ahriman. Some evidence points to important Sasanian adherents, but no independent Zurvanite canon, church, or complete self-description survives.

## Distinctives and comparative notes

The category may represent several speculative tendencies rather than one denomination, and scholars dispute its antiquity, coherence, and place within Zoroastrian history. Hostile outsiders sometimes supplied the fullest narratives, so their symmetry should not be projected onto every Iranian source. The minority-tier branch edge makes that uncertainty explicit: Zurvanite ideas are best studied as contested developments inside Zoroastrian discourse, not as an securely bounded rival religion.
