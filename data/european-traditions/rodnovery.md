---
id: rodnovery
title: Rodnovery
cluster: Esoteric & Modern
color: '#d14fb6'
era_start: late 20th century CE
epistemic_tier: academic_consensus
summary: >-
  Modern Slavic Native Faith movements reconstructing and renewing pre-Christian
  Slavic religion.
canonical_texts:
  - Modern community liturgies
  - Slavic folklore collections
  - Medieval chronicle testimonies
relations:
  - target: slavic-religion
    type: influenced_by
    certainty: academic_consensus
    citation: 'Kaarina Aitamurto, Paganism, Traditionalism, Nationalism (2016)'
origin_year: 1950
origin_geo:
  lat: 50.0647
  lng: 19.945
  place_name: 'Kraków, Lesser Poland'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: Rodnovery, or Slavic Native Faith, comprises modern
    movements that expanded especially after the late Soviet period across
    Russia, Ukraine, Poland, and other Slavic countries and diasporas.
  - >-
    Distinctive Practice: Seasonal festivals, ancestors, land, household
    practice, and gods such as Perun or Veles may be central. The modern archive
    is fragmentary, so reconstruction and innovation are inseparable.
  - >-
    Core Orientation: Modern Slavic Native Faith movements reconstructing and
    renewing pre-Christian Slavic religion.
sources:
  - title: 'Encyclopaedia Britannica: "Rodnovery"'
    url: 'https://www.britannica.com/topic/rodnovery'
  - title: 'Oxford Reference — search results for "Rodnovery"'
    url: 'https://www.oxfordreference.com/search?q=Rodnovery'
  - title: 'Brill — search results for "Rodnovery"'
    url: 'https://brill.com/search?q=Rodnovery'
artifacts:
  - title: "Rodnover open-air sanctuary (kapyshche) on Mount Smrečník, Slovakia"
    imageUrl: "/artifacts/rodnovery-1.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Rodnover_sanctuary_at_Smre%C4%8Dn%C3%ADk,_Slovakia.jpg"
    provenance: "Wojsław Brożyna; CC BY-SA 4.0"
    period: "Image dated 2017"
  - title: "Ritual round dance during Kupala Night celebration in Rakov, Belarus (2015)"
    imageUrl: "/artifacts/rodnovery-2.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Ritual_Round_Dance_duirng_the_Kupala_Night_Celebration_in_Rakov.jpg"
    provenance: "Aliaksei Staliarou; CC BY-SA 4.0"
    period: "Image dated 2015"
---

# Rodnovery

## Historical context

Rodnovery, or Slavic Native Faith, comprises modern movements that expanded especially after the late Soviet period across Russia, Ukraine, Poland, and other Slavic countries and diasporas. Practitioners draw on folklore, archaeology, chronicles, linguistic reconstruction, ecological thought, and new revelation. Organizations vary widely, and no common council determines authentic doctrine or ritual.

## Distinctives and comparative notes

Seasonal festivals, ancestors, land, household practice, and gods such as Perun or Veles may be central. The modern archive is fragmentary, so reconstruction and innovation are inseparable. Some communities promote pluralist or ecological identities; others fuse religion with exclusionary ethnonationalism. Scholarship should neither generalize the latter to every practitioner nor ignore it. The influence relation avoids claiming a continuous pre-Christian institution.
