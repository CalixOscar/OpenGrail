---
id: jainism
title: Jainism
cluster: Dharmic
color: '#f59e0b'
era_start: c. 6th century BCE
epistemic_tier: academic_consensus
summary: >-
  An Indian tradition centered on the Jinas' path of nonviolence, karmic
  purification, disciplined knowledge, and liberation.
canonical_texts:
  - Āgamas
  - Tattvārtha Sūtra
  - Kalpa Sūtra
relations:
  - target: buddhism
    type: parallel_concept
    certainty: academic_consensus
  - target: hinduism
    type: parallel_concept
    certainty: academic_consensus
origin_year: -550
origin_geo:
  lat: 25.8109
  lng: 85.0353
  place_name: 'Pataliputra (Patna), Bihar, India'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: Jain communities trace a succession of twenty-four
    Jinas, with Pārśva and Mahāvīra historically associated with the eastern
    Gangetic world before and during ear...
  - >-
    Distinctive Practice: Jain teaching describes living souls bound by karmic
    matter and liberated through right faith, knowledge, conduct, and severe
    nonviolence. Ascetics model renunc...
  - >-
    Core Orientation: An Indian tradition centered on the Jinas' path of
    nonviolence, karmic purification, disciplined knowledge, and liberation.
sources:
  - title: 'Encyclopaedia Britannica: "Jainism"'
    url: 'https://www.britannica.com/topic/jainism'
  - title: 'Oxford Reference: Overview of Jainism'
    url: 'https://www.oxfordreference.com/search?q=Jainism'
  - title: Routledge Encyclopedia of Indian Philosophy & Dharmic Studies
    url: 'https://www.rep.routledge.com/search?query=Jainism'
artifacts:
  - title: Jainism Historical Sanctuary & Relics
    sourceUrl: 'https://en.wikipedia.org/wiki/Jainism'
    provenance: 'Pataliputra (Patna), Bihar, India'
    period: c. c. 6th century BCE
    description: >-
      Authoritative historical records, sanctuary sites, and material culture
      associated with Jainism.
  - title: Jainism Sacred Scripture & Art
    sourceUrl: 'https://en.wikipedia.org/wiki/Jainism'
    provenance: 'Pataliputra (Patna), Bihar, India'
    period: c. c. 6th century BCE
    description: >-
      Preserved liturgical manuscripts, sacred epigraphy, and canonical
      teachings of Jainism.
---

# Jainism

## Historical context

Jain communities trace a succession of twenty-four Jinas, with Pārśva and Mahāvīra historically associated with the eastern Gangetic world before and during early Buddhism. Monastic and lay networks spread across India, supported literary cultures in Prakrit, Sanskrit, and regional languages, and developed Digambara and Śvetāmbara formations.

## Distinctives and comparative notes

Jain teaching describes living souls bound by karmic matter and liberated through right faith, knowledge, conduct, and severe nonviolence. Ascetics model renunciation; laypeople pursue adapted vows, generosity, pilgrimage, and temple or non-image-based worship. Jainism shares karma and rebirth vocabulary with Buddhist and Hindu traditions but offers a distinct pluralist ontology and should not be treated as a branch of either.
