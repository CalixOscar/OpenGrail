---
id: karma-kagyu
title: Karma Kagyu
cluster: Dharmic
color: '#dc7633'
era_start: 12th century CE
epistemic_tier: academic_consensus
display_weight: 1.35
aliases:
  - Karma Kamtsang
  - Kamtsang Kagyu
summary: >-
  A major Kagyu lineage founded by Düsum Khyenpa and led through the successive
  reincarnation line of the Karmapas.
canonical_texts:
  - Kangyur
  - Tengyur
  - Mahamudra teaching collections
relations:
  - target: kagyu
    type: branch_of
    certainty: academic_consensus
origin_year: 1150
origin_geo:
  lat: 29.7014
  lng: 90.9068
  place_name: 'Lhasa (Potala), U-Tsang, Tibet'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: Karma Kagyu, also called Karma Kamtsang, traces its
    Tibetan institutional formation to Düsum Khyenpa, the First Karmapa, a
    student of Gampopa. Successive Karmapas and associated lineage holders
    developed monasteries, textual traditions, artistic patronage, and retreat
    systems across Tibet and the Himalayas.
  - >-
    Distinctive Practice: The lineage is especially associated with Mahāmudrā,
    the Six Dharmas of Nāropa, guru yoga, and the continuity of oral instruction
    within a shared Tibetan Buddhist curriculum.
  - >-
    Core Orientation: A major Kagyu lineage founded by Düsum Khyenpa and led
    through the successive reincarnation line of the Karmapas.
sources:
  - title: 'Encyclopaedia Britannica: "Karma Kagyu"'
    url: 'https://www.britannica.com/topic/karma-kagyu'
  - title: 'Oxford Reference: Overview of Karma Kagyu'
    url: 'https://www.oxfordreference.com/search?q=Karma%20Kagyu'
  - title: Routledge Encyclopedia of Indian Philosophy & Dharmic Studies
    url: 'https://www.rep.routledge.com/search?query=Karma%20Kagyu'
artifacts:
  - title: "Statue of Rangjung Rigpe Dorje, the 16th Gyalwa Karmapa, wearing the Black Crown"
    imageUrl: "/artifacts/karma-kagyu-1.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:16_Karmapa_Statue_cf_09.jpg"
    provenance: "Secretlondon; CC BY-SA 3.0"
    period: "Image dated 2009"
  - title: "Rumtek Monastery (Dharmachakra Centre) in Sikkim"
    imageUrl: "/artifacts/karma-kagyu-2.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Rumtek_Monastery_-_Sikkim_-_Wikipedia_(1).jpg"
    provenance: "RajashreeTalukdar; CC BY-SA 3.0"
    period: "Image dated 2000"
---

# Karma Kagyu

## Historical context

Karma Kagyu, also called Karma Kamtsang, traces its Tibetan institutional formation to Düsum Khyenpa, the First Karmapa, a student of Gampopa. Successive Karmapas and associated lineage holders developed monasteries, textual traditions, artistic patronage, and retreat systems across Tibet and the Himalayas. Following exile, teachers established substantial international networks; rival recognitions of the seventeenth Karmapa have affected contemporary institutions.

## Distinctives and comparative notes

The lineage is especially associated with Mahāmudrā, the Six Dharmas of Nāropa, guru yoga, and the continuity of oral instruction within a shared Tibetan Buddhist curriculum. Its Karmapa institution is an early and influential reincarnate-lama succession. Disagreements over recognition should be described without declaring one claimant spiritually valid, and Karma Kagyu should not be treated as synonymous with every Kagyu branch.
