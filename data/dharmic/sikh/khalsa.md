---
id: khalsa
title: Khalsa
cluster: Dharmic
color: '#f97316'
era_start: 1699 CE
epistemic_tier: academic_consensus
summary: >-
  The initiated Sikh collective founded by Guru Gobind Singh, marked by shared
  discipline, symbols, courage, and corporate authority.
canonical_texts:
  - Guru Granth Sahib
  - Sikh Rehat Maryada
  - Rahits and historical narratives
relations:
  - target: sikhism
    type: branch_of
    certainty: academic_consensus
origin_year: 1699
origin_geo:
  lat: 31.6962
  lng: 74.9315
  place_name: 'Amritsar, Punjab, India'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: Guru Gobind Singh inaugurated the Khalsa at Anandpur
    in 1699 through initiation with *amrit*, amid conflict and community
    consolidation. Initiates adopted the n...
  - >-
    Distinctive Practice: The Khalsa ideal joins devotion with readiness to
    defend justice and the vulnerable. Initiated Sikhs maintain the five Ks and
    a code governing worship, conduct,...
  - >-
    Core Orientation: The initiated Sikh collective founded by Guru Gobind
    Singh, marked by shared discipline, symbols, courage, and corporate
    authority.
sources:
  - title: 'Encyclopaedia Britannica: "Khalsa"'
    url: 'https://www.britannica.com/topic/khalsa'
  - title: 'Oxford Reference: Overview of Khalsa'
    url: 'https://www.oxfordreference.com/search?q=Khalsa'
  - title: Routledge Encyclopedia of Indian Philosophy & Dharmic Studies
    url: 'https://www.rep.routledge.com/search?query=Khalsa'
artifacts:
  - title: "Guru Gobind Singh founding the Khalsa with the Panj Piare"
    imageUrl: "/artifacts/khalsa-1.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Painting_of_the_establishment_of_the_Khalsa_order_and_Panj_Piare_institution_by_Guru_Gobind_Singh_and_Mata_Jito_at_Anandpur_in_1699,_circa_19th_century.jpg"
    provenance: "Unknown author Unknown author; Public domain"
    period: "19th century"
    description: "This image shows Guru Gobind Singh founding the Khalsa with the Panj Piare, selected as a defining visual reference for Khalsa."
  - title: "Takht Kesgarh Sahib, shrine at the birthplace of the Khalsa"
    imageUrl: "/artifacts/khalsa-2.webp"
    sourceUrl: "https://commons.wikimedia.org/wiki/File:Takhat-Sri-Kesgarh-Sahib-Anandpur-Sahib.jpg"
    provenance: "Deziner89; CC BY-SA 4.0"
    period: "Image dated 2016"
    description: "This image shows Takht Kesgarh Sahib, shrine at the birthplace of the Khalsa, selected as a defining visual reference for Khalsa."
---

# Khalsa

## Historical context

Guru Gobind Singh inaugurated the Khalsa at Anandpur in 1699 through initiation with *amrit*, amid conflict and community consolidation. Initiates adopted the names Singh or Kaur and a shared discipline. Eighteenth-century Khalsa bands, later Sikh polities, colonial military categories, and modern reform all shaped its public form.

## Distinctives and comparative notes

The Khalsa ideal joins devotion with readiness to defend justice and the vulnerable. Initiated Sikhs maintain the five Ks and a code governing worship, conduct, and communal responsibility. Not all Sikhs are formally initiated, but the Khalsa profoundly shapes Sikh identity. Reducing it to a warrior order misses its devotional, ethical, and egalitarian dimensions.
