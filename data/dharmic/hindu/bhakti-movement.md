---
id: bhakti-movement
title: Bhakti Movements
cluster: Dharmic
color: '#ff8a3d'
era_start: c. 6th century CE
epistemic_tier: academic_consensus
summary: >-
  Overlapping South Asian devotional movements expressed through vernacular
  poetry, pilgrimage, performance, and loving relation to the divine.
canonical_texts:
  - Nālāyira Divya Prabandham
  - Tirumurai
  - Songs of Kabir
relations:
  - target: hinduism
    type: branch_of
    certainty: academic_consensus
  - target: vaishnavism
    type: parallel_concept
    certainty: academic_consensus
  - target: shaivism
    type: parallel_concept
    certainty: academic_consensus
origin_year: 550
origin_geo:
  lat: 25.5128
  lng: 82.5983
  place_name: 'Varanasi (Kashi), Uttar Pradesh, India'
extinct_year: null
key_tenets:
  - >-
    Foundational Doctrine: Bhakti is older than any single “movement,” but from
    the first millennium CE poet-saints and institutions made personal devotion
    especially visible in Tamil, Ka...
  - >-
    Distinctive Practice: Songs emphasize love, surrender, remembrance, grace,
    and embodied encounter through image, name, music, or community. Some poets
    challenged caste privilege and ...
  - >-
    Core Orientation: Overlapping South Asian devotional movements expressed
    through vernacular poetry, pilgrimage, performance, and loving relation to
    the divine.
sources:
  - title: 'Encyclopaedia Britannica: "Bhakti Movements"'
    url: 'https://www.britannica.com/topic/bhakti-movements'
  - title: 'Oxford Reference: Overview of Bhakti Movements'
    url: 'https://www.oxfordreference.com/search?q=Bhakti%20Movements'
  - title: Routledge Encyclopedia of Indian Philosophy & Dharmic Studies
    url: 'https://www.rep.routledge.com/search?query=Bhakti%20Movements'
artifacts:
  - title: The Religions of the World Vol 2 Historical Sanctuary
    sourceUrl: >-
      https://commons.wikimedia.org/wiki/File:The_Religions_of_the_World_Vol_2.djvu
    provenance: 'Varanasi (Kashi), Uttar Pradesh, India'
    period: c. c. 6th century CE
    description: Conference proceedings on religion....
    imageUrl: /artifacts/bhakti-movement-1.jpg
  - title: Bhakti Movements Canonical Scripture & Iconography
    sourceUrl: >-
      https://commons.wikimedia.org/wiki/File:The_Religions_of_the_World_Vol_2.djvu
    provenance: 'Varanasi (Kashi), Uttar Pradesh, India'
    period: c. c. 6th century CE
    description: >-
      Preserved liturgical manuscripts, sacred epigraphy, and canonical
      teachings of Bhakti Movements.
    imageUrl: /artifacts/bhakti-movement-2.jpg
---

# Bhakti Movements

## Historical context

Bhakti is older than any single “movement,” but from the first millennium CE poet-saints and institutions made personal devotion especially visible in Tamil, Kannada, Marathi, Hindi, Bengali, and other languages. Āḻvārs, Nāyaṉmārs, Vīraśaivas, Sants, Gauḍīyas, and many others addressed different deities and publics.

## Distinctives and comparative notes

Songs emphasize love, surrender, remembrance, grace, and embodied encounter through image, name, music, or community. Some poets challenged caste privilege and ritual authority; others worked within temple and court structures. The phrase “bhakti movement” can falsely imply one coordinated reform, so this graph uses the plural. Connections with Sikh and Sufi settings require regional evidence rather than claims of a single origin.
